...
현실 속 데이터는 항상 직선 형태로만 표현되지 않으며 곡선 형태를 띠기도 한다. 독립변수의 단항식들을 다항식으로 매핑하여 곡선 형태의 회귀선을 그리는 기법이 다항 회귀분석이다.

## 1) 파이썬 실습: 2차 곡선 모델 피팅

독립변수의 제곱 데이터를 피처에 추가하여 2차 함수 모델을 제작한다.
